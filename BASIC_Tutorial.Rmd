---
title: "Drift Diffusion Modelling using rWiener"
output:
...
![an image caption Source: DDM.](DDM.png)
...
-   Key information on the Maximum likelihood estimation method using
    rWiener
...
variables: dat$q contains the RTs, whereas dat$resp contains a factor to
indicate the response made (upper vs. lower)

```{r}
# set the seed and generat data
set.seed(0)
n_subs = 32
dat <- rwiener(n=n_subs, alpha=2, tau=.3, beta=.5, delta=.5)
head(dat)
```

### d-Wiener-function

-   to get the density of a specific quantile
...
-   the first argument needs to be a probability, indicating the
...
-   this draws the observed lower and upper responses
...
-   2 different alogorithms --\> but they estimate similiar parameters

-   c(1, 0.1, 0.1, 1) --\> starting point from the
...
nlm1 <- nlm(p=c(1, 0.1, 0.1, 1), f=wiener_deviance, dat=dat)
...
## Correlation of RT and Accuracy
...
```{r}
# Add Parameter estimation for multiple conditions here
```
...
## 3.1 RT < 100 or 200ms and RT > 1 or 1.5s;
## maybe choose 200 ms and 1500 ms, since people use the mouse and not the keyboard
#clean_data <- all_data[all_data$q >= .150 & all_data$q <= 2.0, ]

## 3.2 Other outlier correction methods
## Alternate methods for reducing the effect of outliers, such as transforming the data to normalize the data, or using cutoffs based on standard deviation or interquartile range,


# 4. Parameter estimation
# 4.1 Parameter estimation function
...
# 4.2 Calculate the parameters for each subject and store them in a list
params_list <- lapply(split(all_data, all_data$subject), estimate_parameters)

# 4.3 Convert the list to a data frame
params_df <- do.call(rbind, params_list)

# 4.4 Calculate the mean across subjects
...
-   wiener_likelihood: The probability of the observed data under the
    current parameters of the Wiener model.
-   wiener_deviance: A measure of the discrepancy between observed data
    and predictions made by the Wiener model.
-   wiener_aic: A score that balances model fit and complexity, used to
    compare different Wiener models.
-   wiener_bic: Similar to AIC, but places a heavier penalty on model
    complexity.
